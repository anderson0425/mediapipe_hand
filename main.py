...
def hand_gesture(angle_list):
    ...
    if 100000. not in angle_list:
        ...
        #當我把其他手指都彎曲(都收起來)，只有大拇指和食指伸直，則執行if裡面的程式
        ...
        #當我把其他手指都彎曲(都收起來)，只有大拇指伸直，則執行if裡面的程式
        if (angle_list[0]<40) and (angle_list[1]>40) and (angle_list[2]>40) and (angle_list[3]>40) and (angle_list[4]>40):
            gesture_str = "thumb up!"

        #當我把其他手指都彎曲(都收起來)，只有食指伸直，則執行if裡面的程式
        if (angle_list[0]>40) and (angle_list[1]<40) and (angle_list[2]>40) and (angle_list[3]>40) and (angle_list[4]>40):
            gesture_str = "index finger up!"

        #當我把其他手指都彎曲(都收起來)，只有食指和中指伸直，則執行if裡面的程式
        if (angle_list[0]>30) and (angle_list[1]<30) and (angle_list[2]<30) and (angle_list[3]>40) and (angle_list[4]>40):
            gesture_str = "YA!!"

        #當我把所有手指都彎曲(都收起來)呈現石頭，則執行if裡面的程式
        if (angle_list[0]>90) and (angle_list[1]>90) and (angle_list[2]>90) and (angle_list[3]>90) and (angle_list[4]>90):
            gesture_str =  "rock!!"

        #當我把其他手指都彎曲(都收起來)，只有中指伸直，則執行if裡面的程式
        if (angle_list[0]>40) and (angle_list[1]>40) and (angle_list[2]<40) and (angle_list[3]>40) and (angle_list[4]>40):
            gesture_str = "middle finger up!"

        #當我把其他手指都彎曲(都收起來)，只有無名指伸直，則執行if裡面的程式
        if (angle_list[0]>40) and (angle_list[1]>40) and (angle_list[2]>40) and (angle_list[3]<40) and (angle_list[4]>40):
            gesture_str = "ring finger up!"

        #當我把其他手指都彎曲(都收起來)，只有小指伸直，則執行if裡面的程式
        if (angle_list[0]>40) and (angle_list[1]>40) and (angle_list[2]>40) and (angle_list[3]>40) and (angle_list[4]<40):
            gesture_str = "little finger up!"
        ...
        if results.multi_hand_landmarks:  #如果畫面有手，則會回傳一個list，裡面存有很多landmark object(landmark object裡面有x,y,z的數值，皆為-1到1之間)。若沒有手，則回傳none
            for hand_landmarks in results.multi_hand_landmarks:  #每一個hand_landmarks都是一個landmark object，裡面有x,y,z的數值。
                ...
                if keypoint_pos:  
                    ...
                    # -------------------------------------------------------------------------------------------------------------------------
                    # -------------------------------------------------------------------------------------------------------------------------
                    #當比出Nike(大拇指和食指伸直)手勢，則手勢放大。
                    if gesture_str == "Nike":
                        if (int(keypoint_pos[8][0]) < int(keypoint_pos[2][0])):
                            left_up = [int(keypoint_pos[8][0]) - 20 , int(keypoint_pos[8][1]) - 10]
                            right_down = [int(keypoint_pos[2][0]) + 20 , int(keypoint_pos[2][1]) + 10] 
                        if (int(keypoint_pos[8][0]) > int(keypoint_pos[2][0])):
                            right_down = [int(keypoint_pos[8][0]) + 20 , int(keypoint_pos[8][1]) + 10]
                            left_up = [int(keypoint_pos[2][0]) - 20 , int(keypoint_pos[2][1]) - 10]                         
                        frame_processed = enlarge(frame, left_up, right_down)
                        if frame_processed is not None:
                            frame = frame_processed

                    #當只有伸出食指時，則執行if裡面的程式(移動滑鼠游標)
                    if gesture_str == "index finger up!":
                        gesture_str = "mouse control"
                        frame = mouse_track(frame, keypoint_pos)

                    #當只有伸出大拇指時，則執行if裡面的程式(畫面全白)
                    if gesture_str == "thumb up!":
                        img_processing(frame, 11)

                    #當只有伸出無名指時，則執行if裡面的程式(畫面有隨機雜訊)
                    if gesture_str == "ring finger up!":
                        #目前不做任何事
                        s2=[]

                    #當只有伸出小指時，則執行if裡面的程式(打馬賽克)
                    if gesture_str == "little finger up!":
                        #針對小指上的節點([17]....[21])
                        for node in range(17, 21):
                            ROI_x = int(keypoint_pos[node][0])  #某個節點的x值
                            ROI_y = int(keypoint_pos[node][1])  #某個節點的y值

                            left_up = [int(keypoint_pos[node][0]) - 15 , int(keypoint_pos[node][1]) - 10]
                            right_down = [int(keypoint_pos[node][0]) +30 , int(keypoint_pos[node][1]) +50]

                            #按照手指範圍打馬賽克
                            #輸入圖片和馬賽克的左上角和右下角的邊界兩點(X,Y)值
                            frame = mosaic(frame, left_up, right_down)
                            #frame = mosaic(frame , [ROI_x - 15 , ROI_y - 10] , [ROI_x + 30, ROI_y + 50])

                    #當比YA時，則執行if裡面的程式()
                    ##當只有食指和中指伸出，則執行if裡面的程式(點擊)
                    if gesture_str == "YA!!":
                        gesture_str = "click"
                        frame = click(frame, keypoint_pos)
                    if gesture_str == "rock!!":
                        gesture_str = "double_click"
                        frame = double_click(frame, keypoint_pos)
                    # -------------------------------------------------------------------------------------------------------------------------
                    # -------------------------------------------------------------------------------------------------------------------------
