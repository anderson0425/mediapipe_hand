...
TIME_CLICK=0 #用來讓單次點擊不會因為太敏感而點擊太多次。讓它間隔3秒
TIME_DOUBLE_CLICK=0  #用來讓雙擊不會因為太敏感而連續點擊太多次。讓它間隔3秒
...
def BEHAVIOR(frame, gesture_str, keypoint_pos, angle_list, bounding):
    global TIME_CLICK, TIME_DOUBLE_CLICK
    ...
    #當石頭，則不做任何動作，希望把石頭當做使用者的休息時間
    #其他則是避免操控滑鼠時誤認，所以保留
    if gesture_str == "0" and gesture_str == "123" and gesture_str == "235" and gesture_str == "2" and gesture_str == "1" and gesture_str == "12":
        gesture_str = gesture_str + "_rest"

    #當2345，則滑鼠點擊
    #每次點擊間隔3秒
    if gesture_str == "2345":
        LATE_TIME_CLICK = time.time()
        if LATE_TIME_CLICK - TIME_CLICK > 3 or TIME_CLICK==0:  #讓每次單擊都間隔3秒
